...
public class TestEntityQuery2 {
	...
	public static void main(String[] args) {
		...
		CondiParamList condiList = new CondiParamList(sqlTemplate, TestOrclDataType.class);
		condiList.addParam(new CondiParam("appDate", null, CondiParam.OPT_IS_NOT_NULL).setSqlTemplateParamName("date1"));
		condiList.addParam(new CondiParam("appDate", LocalDateTime.now(), CondiParam.OPT_LT).setSqlTemplateParamName("date2"));
		condiList.addParam(new CondiParam("vClob", "FI", CondiParam.OPT_LIKE).setSqlTemplateParamName("content"));
