...
This file "OracleQuery.java" is part of project "pdc-common-tool" , which is belong to Michael Pang (It's Me).
...
/**
 * 这里是关于 Oracle 的查询处理实现类。由于数据库较多，写在一个方法会很难维护，所以拆分开，一个数据库一个类。
...
public class OracleQuery {
	...
	public static final <T> List<T> queryEntity(Connection conn, Class<T> cls, CondiParamList condiList, OrderParamList orderList, PageInfo pageInfo) throws Exception {
		...
		//对于 oracle 数据库的分页处理
		myPagingSql = String.format("select * from (\r\n" + 
